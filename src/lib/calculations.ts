...
function getBestUpstream(resource: Resource, removedTools: Set<string>, removedResources: Set<string>): ProcessingStep[] {
...
function calculateBaseMaterials(
  ...
  while (stack.length > 0) {
    ...
    const inputAmount = Math.ceil(amountToProduce / (inputEfficiency.factor * (bestStep.tool !== 'Refining Oven' ? OGHMIR : 1)));

    stack.push([bestStep.input.name, inputAmount]);

    for (const catalyst of bestStep.catalysts) {
      const catalystAmount = Math.ceil(inputAmount * catalyst.factor * (bestStep.tool !== 'Refining Oven' ? OGHMIR : 1));
      stack.push([catalyst.resource.name, catalystAmount]);
    }

    for (const output of bestStep.outputs) {
      if (output.resource.name !== resourceName) {
        const producedAmount = Math.floor(inputAmount * output.factor * (bestStep.tool !== 'Refining Oven' ? OGHMIR : 1));
        intermediateProducts[output.resource.name] = (intermediateProducts[output.resource.name] || 0) + producedAmount;
      }
    }
  }
...
function getFullProductionChain(
  ...
  while (stack.length > 0) {
    ...
    if (["Granum", "Calx", "Saburra", "Tephra", "Gabore"].includes(resourceName) || !resources[resourceName]) {
      continue;
    }
...
function combineSteps(chain: [string, number, string, [string, number][], string, number][]): [{ [key: string]: number }, number, string, [string, number][], string][] {
...
function formatProductionSteps(chain: [string, number, string, [string, number][], string, number][]): string {
  ...
  for (const [outputs, totalOutput, inputResource, catalysts, tool, efficiency] of combinedChain) {
    const catalystStr = catalysts.length > 0
      ? ` with ${catalysts.map(([name, amount]) => `${amount} ${name}`).join(" and ")}`
      : "";
    const inputAmount = Math.ceil(totalOutput / efficiency);
    const outputStr = Object.entries(outputs).map(([resource, amount]) => `${amount} ${resource}`).join(", ");
...
export function calculateResources(
  ...
  try {
    ...
    const baseMaterials = calculateBaseMaterials(targetResource, targetAmount, removedToolsSet, removedResourcesSet, isOghmir);
    const chain = getFullProductionChain(targetResource, targetAmount, removedToolsSet, removedResourcesSet, isOghmir);
